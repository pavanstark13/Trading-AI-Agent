...
class MarketStructureType(StrEnum):
...
class MarketStructureStrategy(BaseStrategy):
    ...
    def _detect_structure_events(
        ...
        for _i, swing in enumerate(all_swings):
            ...
            if swing.point_type == "high":
                ...
                event_type = (
                    MarketStructureType.CHOCH
                    if prev_event_direction == "bearish"
                    else MarketStructureType.BOS
                )
                events.append(
                    StructureEvent(
                        index=break_idx,
                ...
                event_type = (
                    MarketStructureType.CHOCH
                    if prev_event_direction == "bullish"
                    else MarketStructureType.BOS
                )
                events.append(
                    StructureEvent(
    ...
    def _find_break(
        ...
        for i in range(swing.index + 1, len(candles)):
            if (
                swing.point_type == "high"
                and closes[i] > swing.price
                or swing.point_type == "low"
                and closes[i] < swing.price
            ):
                return i
        return None
